...
const SITE_DOMAINS = ["com", "org", "net", "io", "co", "app", "dev", "tech", "biz"];
...
function generateSiteId() {
	...
	}-${Math.floor(Math.random() * 9999)}.${domain}`;
}

function generateAppVersion() {
	return `${Math.floor(Math.random() * 5) + 13}.${Math.floor(Math.random() * 20)}.${Math.floor(
		Math.random() * 10
	)}`;
}
...
function makeEvent() {
	...
	return {
		...
		properties: {
			...
			site_plan: ["free", "starter", "business", "enterprise"][
